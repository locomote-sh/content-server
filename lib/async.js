...
const Log = require('log4js').getLogger('async');
...
function queue( queueName, fn ) {
    return new Promise( async ( resolve, reject ) => {
        ...
        // If new queue then add to the lookup.
        if( queue.length === 0 ) {
            Queues[queueName] = queue;
        }
        // Push the function and the promise callbacks onto the queue.
        queue.push({ resolve, reject, fn });
        // If queue length is greater than one then queue processing
        // has already started, so return.
        if( queue.length > 1 ) {
            return;
        }
        // To be here queue length must be equal to one - i.e. the first
        // op has just been added to the queue; so start queue processing
        // and continue until the queue is empty (additional items may
        // be added to the end of the queue whilst the first and subsequent
        // items are being processed).
        while( queue.length > 0 ) {
            // Read the first item from the queue.
            const { resolve, reject, fn } = queue[0];
        ...
        // Queue processing completed, delete the queue.
