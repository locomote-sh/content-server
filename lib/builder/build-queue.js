...
function start( core, manifests, branchdb, buildb, settings, server ) {
    ...
    const {
        ...
        // A pattern for generating the content origin.
        contentOriginFormat = '{repoPath}'
    ...
    const makeOrigin = info => TT.eval( contentOriginFormat, info );
    ...
    async function doBuild( info ) {
        ...
        try {
            ...
            const origin   = makeOrigin( info );
            Log.debug('Content origin:', origin );
            // The repo's workspace directory.
            const buildDir = Path.join( workspaceHome, account, repo );
            // The build environment.
            const { env }  = process;

            // Read build profile information from the manifest.
            manifests.invalidateCache( repoPath );
            ...
            switch( typeof profile ) {
                case 'undefined':
                    Log.debug('No build profile defined for %s', key );
                    await buildb.addBuildCompletion( account, repo, branch, commit, 'NO_BUILD_PROFILE');
                    return;
                case 'string':
                    ...
                    if( !profile ) {
                        Log.debug('Build profile "%s" not found for %s', profileID, key );
                        return;
                    }
                    break;
            }
            ...
            if( !buildable.includes( branch ) ) {
                ...
                await buildb.addBuildCompletion( account, repo, branch, commit, 'NOT_BUILDABLE');
                return;
            }
            ...
            if( code === 0 ) {
                ...
                await buildb.addBuildCompletion( account, repo, branch, commit, 'BUILT');
