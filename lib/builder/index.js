...
service.initServer = function( settings, core, manifests, branchdb ) {
    ...
    const buildb = require('./builddb').make( settings );

    const buildQueue = require('./build-queue')
        .start( core, manifests, branchdb, buildb, settings, server );
    ...
    function notifyContentUpdate( info, source = 'internal' ) {
        try {
            if( source == 'external' || settings.get('build.handleInternalCUNotifications') ) {
    ...
    function parseBuildKey( key ) {
        const [ account, repoDir, branch ] = key.split('/');
        // Strip trailing .git from repo name, if any.
        const repo = repoDir.replace(/\.git$/,'');
    ...
    try {
        ...
        const { host, port } = settings.get('build.updatesListener');
        ...
        .listen( port, host );
        Log.info('Listening for content update notifications on %s:%d', host, port );
    }
    catch( e ) {
    ...
    require('./startup-check').run( this, branchdb, buildb );
